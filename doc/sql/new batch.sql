-- 该批量执行内容为: queue、task、job、step
-- 一个queue多个task, 一个task对应多个job, 一个task对应多个step
-- 定时任务时以TASK定义为主. 查询出定时的TASK. TASK-JOB-STEP
-- 队列任务以queue为主.QUEUE-TASK-JOB-STEP
-- 每个job之间可以并发执行, queue、task、step仅可以串行执行.
-- 最小单位为step,创建step的目的是为了防止一个job内存在一组任务(step)需要串行执行并且与当前运行的任务并发执行.
...
create table if not exists batch_queue_definition (
	...
	queue_exc_flag int(1) not null default "0" comment "任务执行标志. 0-手动, 1-自动, 2-手/自.",
	queue_trigger_id varchar(20) comment "关联触发器ID",
	queue_executor varchar(200) not null comment "队列执行器",
	queue_init_value varchar(500) comment "队列初始值",
...
create index batch_queue_definition_index2 on batch_queue_definition(queue_exc_flag);
...
drop table if exists batch_queue_execute;
create table batch_queue_execute(
	execute_queue_id bigint(20) not null comment "执行队列ID",
	execute_queue_name varchar(32) comment "执行对列名称",
	execute_task_id bigint(20) not null comment "执行任务ID",
	execute_task_name varchar(32) comment "执行任务名称",
	execute_num int(5) not null comment "任务执行序号",
	create_date varchar(8) not null comment "创建时间",
	valid_status varchar(1) not null default "0" comment "有效状态. 0-是, 1-否, D-已废弃",
	remark varchar(500) comment "备注"
) character set utf8 collate utf8_general_ci comment "批量队列运行表";
create index batch_queue_execute_index1 on batch_queue_execute(execute_queue_id);
...
create table if not exists batch_task_definition(
	...
	task_auto_flag int(1) not null default "1" comment "是否自动执行. 0-是, 1-否",
	task_associate_queue_id bigint(20) comment "关联队列ID",
...
create index batch_task_definition_index2 on batch_task_definition(task_associate_queue_id);
create index batch_task_definition_index3 on batch_task_definition(task_auto_flag);
...
create table if not exists batch_job_definition(
	...
	job_log_flag int(1) default "0" comment "日志等级标记. 0-登记, 1-不登记",
	...
	job_pause_flag int(1) default "1" comment "可暂停标志, 0-是, 1-否",
...
drop table if exists batch_executor_seqlog;
create table batch_executor_seqlog(
	seq_id bigint(20) not null comment "序号ID",
	job_id bigint(20) not null comment "关联作业ID",
	job_name varchar(32) comment "作业名称",
	status varchar(1) comment "执行状态",
	create_date varchar(8) comment "创建时间",
	time_stamp bigint(12) comment "时间戳",
	valid_status varchar(1) not null default "0" comment "有效状态. 0-是, 1-否",
	remark varchar(500) comment "备注"
) character set utf8 collate utf8_general_ci comment "异步作业执行序列日志表";
create UNIQUE INDEX batch_executor_seqlog_index1 on batch_executor_seqlog (seq_id);
...
create table if not exists batch_task_execution_log_register(
	...
	task_job_status VARCHAR(20) COMMENT "任务状态,COMPLETED-处理中, SUCCSS-成功, FAILED-失败,UNKOWN-非正常状态",
...
create table if not exists batch_task_execution_log_history(
	...
	history_task_job_status VARCHAR(20) COMMENT "任务状态,COMPLETED-处理中, SUCCSS-成功, FAILED-失败,UNKOWN-非正常状态",
...
create table if not exists batch_step_execution_log_register(
	...
	step_job_status VARCHAR(20) COMMENT "步骤状态,COMPLETED-处理中, SUCCSS-成功, FAILED-失败,UNKOWN-非正常状态",
...
create table if not exists batch_step_execution_log_history(
	...
	history_step_job_status VARCHAR(20) COMMENT "步骤状态,COMPLETED-处理中, SUCCSS-成功, FAILED-失败,UNKOWN-非正常状态",
...
create table if not exists batch_timing_task_log_register(
	...
	timing_job_status VARCHAR(20) COMMENT "定时任务状态,COMPLETED-处理中, SUCCSS-成功, FAILED-失败,UNKOWN-非正常状态",
...
create table if not exists batch_timing_task_log_history(
	...
	history_timing_job_status VARCHAR(20) COMMENT "定时任务状态,COMPLETED-处理中, SUCCSS-成功, FAILED-失败,UNKOWN-非正常状态",
